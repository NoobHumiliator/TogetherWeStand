        ...
        if entUnit then
            if entUnit:IsCreature() then
                if bIsChampion then
                    ...
                    local nParticle = ParticleManager:CreateParticle("particles/units/heroes/hero_bloodseeker/bloodseeker_bloodrage.vpcf", PATTACH_ABSORIGIN_FOLLOW, entUnit)
            ...
            -- 在简单和普通难度所有玩家平分怪物经验
            if self._gameRound._gameMode.map_difficulty < 3 then
