...
function modifier_item_unhallowed_icon_effect:OnAttackLanded(params)
    if IsServer() then
        ...
        if Attacker ~= nil and Attacker == self:GetParent() and Target ~= nil then
            local allies = FindUnitsInRadius(Attacker:GetTeamNumber(), self:GetCaster():GetOrigin(), nil, 1500, DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, FIND_ANY_ORDER, false)
            for _, ally in pairs(allies) do
                if ally:FindModifierByName("modifier_item_unhallowed_icon_effect") then
