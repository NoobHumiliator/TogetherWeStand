...
function modifier_ogreseal_flop:OnCreated(kv)
    if IsServer() then
        ...
        self.flInitialVelocityZ = math.sqrt(2 * flArcDeltaZ * OGRE_ACCELERATION_Z * self.nHopCount)

        local flDeltaZ = self.vLastKnownTargetPos.z - self.vStartPosition.z
        local flSqrtDet = math.sqrt(math.max(0, (self.flInitialVelocityZ * self.flInitialVelocityZ) - 2 * OGRE_ACCELERATION_Z * self.nHopCount * flDeltaZ))
