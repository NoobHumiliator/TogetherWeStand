...
function EnvironmentController:ApplyBlindModifier()
    ...
    Timers:CreateTimer({
        endTime = 1,
        callback = function()
            local enemies = FindUnitsInRadius(DOTA_TEAM_BADGUYS, Vector(0, 0, 0), nil, -1, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_ALL, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, FIND_ANY_ORDER, false)
...
function EnvironmentController:SpawnLightBall()
    ...
    Timers:CreateTimer({
        ...
        callback = function()
            ...
            CHoldoutGameMode._currentRound._environmentcontroller.ballInterval = CHoldoutGameMode._currentRound._environmentcontroller.ballInterval + 2
            if CHoldoutGameMode._currentRound == nil or CHoldoutGameMode._currentRound._alias ~= "darkness" or CHoldoutGameMode._currentRound._environmentcontroller.blindEnd then
                return nil
            else
...
function EnvironmentController:SpawnTombStone()
    ...
    Timers:CreateTimer({
        ...
        callback = function()
            ...
            local enemies = FindUnitsInRadius(DOTA_TEAM_BADGUYS, Vector(0, 0, 0), nil, FIND_UNITS_EVERYWHERE, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, FIND_ANY_ORDER, false)

            if #enemies > 0 then
                local hero = RandomInt(1, #enemies)
                for i = 1, 4 do
                    local hero = enemies[RandomInt(1, #enemies)]  --找一个随机英雄 对其周围放置方尖塔
...
function EnvironmentController:ApplyBeaconModifier()  --随机选取一个无面者加暗影信标
    ...
    Timers:CreateTimer({
        ...
        callback = function()
            ...
            --print("wfffds")
            local units = FindUnitsInRadius(DOTA_TEAM_BADGUYS, Vector(0, 0, 0), nil, -1, DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_ALL, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES + DOTA_UNIT_TARGET_FLAG_INVULNERABLE, FIND_ANY_ORDER, false)
...
function RandomHeroIgnoreImmnueAndInvulnerable()
    local enemies = FindUnitsInRadius(DOTA_TEAM_BADGUYS, Vector(0, 0, 0), nil, FIND_UNITS_EVERYWHERE, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES + DOTA_UNIT_TARGET_FLAG_INVULNERABLE, FIND_ANY_ORDER, false)
    if #enemies > 0 then
        local index = RandomInt(1, #enemies)
        return enemies[index]
    else
