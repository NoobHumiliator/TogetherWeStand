...
modifier_ogre_magi_item_multicast_lua_proc = class({})

--------------------------------------------------------------------------------
-- Classifications
function modifier_ogre_magi_item_multicast_lua_proc:IsHidden()
    return false
end

function modifier_ogre_magi_item_multicast_lua_proc:IsDebuff()
    return false
end

function modifier_ogre_magi_item_multicast_lua_proc:IsStunDebuff()
    return false
end

function modifier_ogre_magi_item_multicast_lua_proc:IsPurgable()
    return true
end

function modifier_ogre_magi_item_multicast_lua_proc:GetAttributes()
    return MODIFIER_ATTRIBUTE_MULTIPLE
end

--------------------------------------------------------------------------------
-- Initializations
function modifier_ogre_magi_item_multicast_lua_proc:OnCreated(kv)
    ...
    if self.target_type == DOTA_UNIT_TARGET_CUSTOM then
        self.target_type = DOTA_UNIT_TARGET_BASIC
    end
...
function modifier_ogre_magi_item_multicast_lua_proc:OnRefresh(kv)

end

function modifier_ogre_magi_item_multicast_lua_proc:OnRemoved()
end

function modifier_ogre_magi_item_multicast_lua_proc:OnDestroy()
end

--------------------------------------------------------------------------------
-- Interval Effects
function modifier_ogre_magi_item_multicast_lua_proc:OnIntervalThink()
...
--------------------------------------------------------------------------------
-- Graphics & Animations
function modifier_ogre_magi_item_multicast_lua_proc:PlayEffects(value)
    local particle_target = self.caster
    if self.single then
        particle_target = self.target
    end
    value = value + 1
    ...
    -- Create Particle
    local effect_cast = ParticleManager:CreateParticle(particle_cast, PATTACH_OVERHEAD_FOLLOW, particle_target)
    ...
    if sound > 0 then
        EmitSoundOn(sound_cast, particle_target)
    end
end