...
--------------------------------------------------------------------------------
modifier_ogre_magi_item_multicast_lua = class({})
modifier_ogre_magi_item_multicast_lua.singles = {
    ["ogre_magi_fireblast"] = true,
    ["ogre_magi_unrefined_fireblast"] = true,
}

modifier_ogre_magi_item_multicast_lua.ability_blacklist = {
    ["huskar_life_break"] = true,
    ["frostivus2018_huskar_life_break"] = true,
...
--------------------------------------------------------------------------------
-- Classifications
function modifier_ogre_magi_item_multicast_lua:IsHidden()
    return false
end

function modifier_ogre_magi_item_multicast_lua:IsDebuff()
    return false
end

function modifier_ogre_magi_item_multicast_lua:IsPurgable()
    return false
end

--------------------------------------------------------------------------------
-- Initializations
function modifier_ogre_magi_item_multicast_lua:OnCreated(kv)
    -- references
...
function modifier_ogre_magi_item_multicast_lua:OnRefresh(kv)
    -- references
    self.chance_2 = self:GetAbility():GetSpecialValueFor("multicast_2_times")
    self.chance_3 = self:GetAbility():GetSpecialValueFor("multicast_3_times")
    self.chance_4 = self:GetAbility():GetSpecialValueFor("multicast_4_times")
...
function modifier_ogre_magi_item_multicast_lua:OnRemoved()
end

function modifier_ogre_magi_item_multicast_lua:OnDestroy()
end

--------------------------------------------------------------------------------
-- Modifier Effects
function modifier_ogre_magi_item_multicast_lua:DeclareFunctions()
    local funcs = {
...
function modifier_ogre_magi_item_multicast_lua:OnAbilityFullyCast(params)
    ...
    if params.ability:IsItem() and params.target:GetTeamNumber() == self:GetCaster():GetTeamNumber() then return end
    if self.ability_blacklist[params.ability:GetAbilityName()] == true then return end
    ...
    "modifier_ogre_magi_item_multicast_lua_proc", -- modifier name
    {
