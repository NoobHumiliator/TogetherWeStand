...
function modifier_dark_seer_wall_of_replica_nb2017:OnCreated(kv)
    ...
    if self:GetCaster():HasScepter() then
        self.replica_damage_outgoing = self:GetAbility():GetSpecialValueFor("replica_damage_outgoing_scepter")
    end
    ...
    if IsServer() then
        ...
        self:GetParent():EmitSoundParams("Hero_Dark_Seer.Wall_of_Replica_lp", 100, 0, self:GetDuration())
        ...
        if #enemies > 0 then
            for _, enemy in pairs(enemies) do
                ...
                if bTryCreate == true then
                    ...
                    if flDistOffLine <= (enemy:GetPaddedCollisionRadius() + 50) then
                        ...
                        local replica = CreateUnitByName(enemy:GetUnitName(), enemy:GetOrigin() + RandomVector(enemy:GetPaddedCollisionRadius() + 50), true, self:GetCaster(), self:GetCaster(), self:GetCaster():GetTeamNumber())
