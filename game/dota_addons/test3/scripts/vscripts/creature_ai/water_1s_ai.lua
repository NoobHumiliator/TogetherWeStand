...
function BehaviorEvolve:Evaluate()
	...
	if currentBehavior == self then return desire end
	self.plasmaAbility = thisEntity:FindAbilityByName( "water_evolve_to_2s" )                          
    if thisEntity:GetMaxHealth()>30000*flDHPadjust and self.plasmaAbility and self.plasmaAbility:IsFullyCastable() then
          desire=15
    end 
    return desire
end
