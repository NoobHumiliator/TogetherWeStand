...
function BossRubickThink()
    if IsServer() then
        ...
        if s_Dagon == nil or s_Sheep == nil then
            FindItems()
        end

        if CheckToCastDagon(thisEntity, s_Dagon) == true then
            CastDagon(thisEntity, s_Dagon)
            return 0.1
        end
...
function FindItems()
    for i = 0, DOTA_ITEM_MAX - 1 do
        ...
        if item then
            if item:GetAbilityName() == "item_dagon_5" then
                s_Dagon = item
            end
...
function CheckToCastDagon(caster, ability)
...
function CastDagon(caster, ability)
    local enemies = FindUnitsInRadius(caster:GetTeamNumber(), caster:GetOrigin(), caster, 800, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, 0, false)
    if #enemies > 0 then
        ExecuteOrderFromTable({
