...
function Spawn( entityKeyValues )
    ...
    behaviorSystem = AICore:CreateBehaviorSystem( { BehaviorNone , BehaviorHold , BehaviorFreeze ,BehaviorEvolve ,BehaviorStrike,BehaviorStatic_Link} ) 
end
...
function BehaviorStrike:Evaluate()
	self.strikeAbility = thisEntity:FindAbilityByName("avalance")    --对技能进行定义
	local target
	local desire = 0
...
function BehaviorEvolve:Evaluate()
	local desire = 1
	if currentBehavior == self then return desire end
	self.plasmaAbility = thisEntity:FindAbilityByName( "tiny_growto_4" )                          
    if thisEntity:GetHealth()>17500 and self.plasmaAbility and self.plasmaAbility:IsFullyCastable() then
          desire=15
    end 
    return desire
end
...
function BehaviorStatic_Link:Evaluate()
    ...
    local enemies = FindUnitsInRadius( DOTA_TEAM_GOODGUYS, thisEntity:GetOrigin(), nil, range, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_ALL, 0, 0, false )
	local minHP = nil
	local target = nil
...
--------------------------------------------------------------------------------------------------------
AICore.possibleBehaviors = { BehaviorNone , BehaviorHold , BehaviorFreeze ,BehaviorEvolve,BehaviorStrike,BehaviorStatic_Link}
