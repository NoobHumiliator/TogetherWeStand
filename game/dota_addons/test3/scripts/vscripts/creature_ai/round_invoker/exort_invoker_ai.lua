--[[ Exort InvokerAI ]]
require("creature_ai/round_invoker/invoker_shared_ai")
...
function ExortInvokerThink()
    if IsServer() then
        ...
        local forgedSpirits = Entities:FindAllByName("npc_dota_invoker_forged_spirit")
        if #forgedSpirits > 0 then
            local enemies = GetVisibleEnemyHeroesInRange(thisEntity, -1)
            if #enemies > 0 then
                for _, forgedSpirit in pairs(forgedSpirits) do
        ...
        if CheckToCastEuls(thisEntity, s_Euls) then
            CastEuls(thisEntity, s_Euls)
            return 0.1
        end

        if CheckToCastBlink(thisEntity, s_Blink) then
            CastBlink(thisEntity, s_Blink)
            return 0.1
        end

        if s_ActiveSpell2:IsFullyCastable() then
            if s_ActiveSpell2 == s_SunStrike and CheckToCastSunStrike(thisEntity, s_ActiveSpell2) then
                if CastSunStrike(thisEntity, s_ActiveSpell2) then
                    return 0.1
                end
            end

            if s_ActiveSpell2 == s_IceWall and CheckToCastIceWall(thisEntity, s_ActiveSpell2) then
                CastIceWall(thisEntity, s_ActiveSpell2)
                return 0.1
            end

            if s_ActiveSpell2 == s_Meteor and CheckToCastMeteor(thisEntity, s_ActiveSpell2) then
                CastMeteor(thisEntity, s_ActiveSpell2)
                return 0.1
            end

            if s_ActiveSpell2 == s_ForgedSpirit and CheckToCastForgedSpirit(thisEntity, s_ActiveSpell2) then
        ...
        if s_ActiveSpell1:IsFullyCastable() then
            if s_ActiveSpell1 == s_IceWall and CheckToCastIceWall(thisEntity, s_ActiveSpell1) then
                CastIceWall(thisEntity, s_ActiveSpell1)
                return 0.1
            end

            if s_ActiveSpell1 == s_SunStrike and CheckToCastSunStrike(thisEntity, s_ActiveSpell1) then
                if CastSunStrike(thisEntity, s_ActiveSpell1) then
                    return 0.1
                end
            end

            if s_ActiveSpell1 == s_Meteor and CheckToCastMeteor(thisEntity, s_ActiveSpell1) then
                CastMeteor(thisEntity, s_ActiveSpell1)
                return 0.1
            end

            if s_ActiveSpell1 == s_ForgedSpirit and CheckToCastForgedSpirit(thisEntity, s_ActiveSpell1) then
