...
function BossInvokerThink()
    if IsServer() then
        ...
        local forgedSpirits = Entities:FindAllByName("npc_dota_creature_boss_invoker_forged_spirit")
...
function InvokeThink()
    ...
    local targets = GetEnemyHeroesInRange(thisEntity, FIND_UNITS_EVERYWHERE)

    if #targets > 0 and thisEntity:GetAggroTarget() == nil then

        local target = targets[1]
        local order = DOTA_UNIT_ORDER_MOVE_TO_POSITION
        if target:CanEntityBeSeenByMyTeam(thisEntity) then
