...
function tiny_wake(keys)
    ...
    local targets = FindUnitsInRadius(c_team, caster:GetOrigin(), nil, -1, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_ALL, 0, FIND_ANY_ORDER, false)  --全地图所有单位
...
function remove_wake_ability(keys)
    local argets = FindUnitsInRadius(DOTA_TEAM_GOODGUYS, Vector(0, 0, 0), nil, -1, DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_ALL, 0, FIND_ANY_ORDER, false)
    local tiny_5 = nil
    for _, unit in pairs(argets) do
        if unit:GetUnitName() == "npc_dota_tiny_5" then
            if unit and unit:HasAbility("tiny_wake") then
                unit:RemoveAbility("tiny_wake")
            end
        end
    end
end