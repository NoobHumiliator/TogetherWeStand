...
LinkLuaModifier("modifier_storegga_thrown", "creature_ability/round_elder_titan/modifier_storegga_thrown", LUA_MODIFIER_MOTION_BOTH)
        ...
        for i = 1, self.children_to_spawn do
            if #self.hChildren < self.max_children then
                ...
                if hChild ~= nil and hChild:IsNull() == false then
                    ...
                    local farthestHero = nil
                    local enemies = FindUnitsInRadius(DOTA_TEAM_BADGUYS, hChild:GetOrigin(), nil, -1, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, FIND_FARTHEST, false)
                    if #enemies > 0 then
                        farthestHero = enemies[1]
                    end
                    hChild:SetInitialGoalEntity(farthestHero)
                    hChild:SetDeathXP(0)
