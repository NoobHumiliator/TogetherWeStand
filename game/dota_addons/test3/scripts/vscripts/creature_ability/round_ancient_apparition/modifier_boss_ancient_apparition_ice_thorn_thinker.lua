...
function modifier_boss_ancient_apparition_ice_thorn_thinker:OnCreated(kv)
    ...
    if IsServer() then
        ...
        ParticleManager:SetParticleControl(nFXIndex, 1, Vector(self.radius, self.delay, 1))
...
function modifier_boss_ancient_apparition_ice_thorn_thinker:OnIntervalThink()
    if IsServer() then
        if self:GetCaster():IsAlive() then
            ...
            local enemies = FindUnitsInRadius(self:GetParent():GetTeamNumber(), self:GetParent():GetOrigin(), self:GetParent(), self.radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_NONE, FIND_ANY_ORDER, false)
