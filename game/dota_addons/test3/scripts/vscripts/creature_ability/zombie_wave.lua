LinkLuaModifier( "modifier_zombie_explode_debuff", "creature_ability/modifier/modifier_zombie_explode_debuff", LUA_MODIFIER_MOTION_NONE )
require( "util" )
...
function LaunchWave( event )
    ...
    for i=1,#directions do
        ...
        local units=FindUnitsInLine(DOTA_TEAM_BADGUYS,casterOrigin+direction*100,casterOrigin+direction*7500,nil,110,DOTA_UNIT_TARGET_TEAM_ENEMY,DOTA_UNIT_TARGET_HERO+DOTA_UNIT_TARGET_BASIC,DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES)
        for _,unit in pairs(units) do

            unit:Purge(true,false,false,false,true) --净化
            RemoveDurableBuff(unit) --移除强保命技能
            ...
            ApplyDamage( {
              ...
              attacker  = caster,
              ability   =  ability,
              damage    = unit:GetMaxHealth()*0.05*statck,
