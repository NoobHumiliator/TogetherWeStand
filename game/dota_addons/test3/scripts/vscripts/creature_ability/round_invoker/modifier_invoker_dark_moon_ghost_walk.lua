    ...
    if IsServer() then
        ...
        for _, hForgedSpirit in pairs(SUMMONED_UNITS) do
            if not hForgedSpirit:IsNull() and hForgedSpirit:GetAggroTarget() == nil then
                local enemies = GetVisibleEnemiesInRange(hForgedSpirit, -1)
                if #enemies > 0 then
...
function modifier_invoker_dark_moon_ghost_walk:BeginPhase(phase)
    if IsServer() then
        ...
        if phase == PHASE_WEX then
            ...
            ParticleManager:SetParticleControl(self.nFXIndex, 2, Vector(20, 20, 20))
            ParticleManager:SetParticleControl(self.nFXIndex, 3, Vector(self.aura_radius, self.aura_radius, self.aura_radius))
            self:AddParticle(self.nFXIndex, false, false, -1, false, false)

            self:GetCaster():EmitSoundParams("Hero_Razor.Storm.Loop", 0, 0.75, 20)
        end
...
function modifier_invoker_dark_moon_ghost_walk:OnDeath(params)
    if IsServer() then
        ...
        if hUnit:GetUnitName() == "npc_dota_creature_boss_invoker_forged_spirit" then
        ...
        for _, vPosition in pairs(SPAWNER_POSITIONS) do
            for i = 1, 4 do
                if #SUMMONED_UNITS < MAX_FORGED_SPIRIT_COUNT then
                    local hForgedSpirit = CreateUnitByName("npc_dota_creature_boss_invoker_forged_spirit", vPosition, true, self:GetCaster(), self:GetCaster(), self:GetCaster():GetTeamNumber())
...
function modifier_invoker_dark_moon_ghost_walk:ExortPhase_CreateUnitsNearby()
    ...
    if not hForgeSpiritAbility:IsNull() then
        for _, vPos in pairs(randomPositionsNearby) do
            if #SUMMONED_UNITS < MAX_FORGED_SPIRIT_COUNT then
                local hForgedSpirit = CreateUnitByName("npc_dota_creature_boss_invoker_forged_spirit", vPos, true, self:GetCaster(), self:GetCaster(), self:GetCaster():GetTeamNumber())
                table.insert(SUMMONED_UNITS, hForgedSpirit)
                bSpellCast = true
            end
        end
...
function modifier_invoker_dark_moon_ghost_walk:ExortPhase_Meteor(enemies)
    ...
    for i = 1, nMeteors do
        ...
        if hMeteor ~= nil then
            ...
            local vRandom = RandomVector(700)
        ...
        for _, hEnemy in pairs(enemies) do
            for i = 1, 4 do
                if #NORMAL_FORGED_SPIRITS < 10 then
                    ...
                    local hForgedSpirit = CreateUnitByName("npc_dota_creature_boss_invoker_forged_spirit", hEnemy:GetAbsOrigin() + vRandomOffset, true, self:GetCaster(), self:GetCaster(), self:GetCaster():GetTeamNumber())
