...
function modifier_amoeba_boss_passive:CheckState()
    local state = {}
    if IsServer() then
        if self:GetParent():GetUnitName() == "npc_dota_creature_amoeba_boss" then
...
function modifier_amoeba_boss_passive:OnStackCountChanged(nOldCount)
    if IsServer() then
        ...
        self:GetParent():SetHullRadius(8 + self:GetStackCount() * self.hull_radius_per_stack)
        FindClearSpaceForUnit(self:GetParent(), self:GetParent():GetOrigin(), false)

        if self:GetStackCount() <= 50 and self:GetStackCount() > 25 and self:GetParent():GetUnitName() ~= "npc_dota_creature_amoeba_boss" then
            self:GetParent():SetRenderColor(0, 255, 0)
            self:GetParent():SetMaxHealth(12000)
            self:GetParent():Heal(12000, self)
        end
...
function modifier_amoeba_boss_passive:GetModifierBaseAttackTimeConstant(params)
    return 1 + (self:GetStackCount() * self.base_attack_time_per_stack)
end
