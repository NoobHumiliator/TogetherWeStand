function IsNearDarkCheck(event)
    ...
    if ability then
        ...
        local fire_twin_enemies = FindUnitsInRadius(DOTA_TEAM_BADGUYS, target:GetOrigin(), nil, cancel_range, DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_ALL, 0, FIND_ANY_ORDER, false)
        ...
        local friends = FindUnitsInRadius(target:GetTeam(), target:GetOrigin(), nil, target.fire_twin_debuff_range, DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_ALL, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, FIND_ANY_ORDER, false)
