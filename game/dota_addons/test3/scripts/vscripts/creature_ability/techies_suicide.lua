...
LinkLuaModifier( "modifier_tinker_creature_techies_suicide_leap", "creature_ability/modifier/modifier_tinker_creature_techies_suicide_leap", LUA_MODIFIER_MOTION_BOTH )
--------------------------------------------------------------------------------

function tinker_creature_techies_suicide:OnAbilityPhaseStart()
	if IsServer() then
		local hp_cost = self:GetSpecialValueFor( "hp_cost" )
		if self:GetCaster():GetHealth() < ( self:GetCaster():GetMaxHealth() * hp_cost / 100.0 ) then
...
function tinker_creature_techies_suicide:BlowUp()
	if IsServer() then
		...
		--print("sdadsadasd")
		local enemies = FindUnitsInRadius( self:GetCaster():GetTeamNumber(), self:GetCaster():GetOrigin(), self:GetCaster(), radius, DOTA_UNIT_TARGET_TEAM_BOTH, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, 0, false )
		if #enemies > 0 then
