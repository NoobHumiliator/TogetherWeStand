...
function SnowballStart(keys)
    ...
    local tusks= {}
    local allies = FindUnitsInRadius( caster:GetTeam(), caster:GetOrigin(), nil, -1, DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_ALL, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, FIND_ANY_ORDER, false )
    ...
    Timers:CreateTimer({
			endTime = 0.5,
			callback = function()
            local allEnemies = FindUnitsInRadius( DOTA_TEAM_BADGUYS, Vector(0,0,0), nil, -1, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_ALL, 0, FIND_ANY_ORDER, false )  --全地图所有英雄
...
function SnowballDied(event)
   ...
   local allies= FindUnitsInRadius( caster:GetTeam(), Vector(0,0,0), nil, -1, DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_ALL, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, FIND_ANY_ORDER, false )
   for _,ally in pairs(allies) do
   ...
   if caster.snowballOwner:GetUnitName()=="npc_dota_tusk_boss" then --如果打错了雪球
   	  local allies = FindUnitsInRadius( DOTA_TEAM_BADGUYS, Vector(0,0,0), nil, -1, DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_ALL, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, FIND_ANY_ORDER, false )
