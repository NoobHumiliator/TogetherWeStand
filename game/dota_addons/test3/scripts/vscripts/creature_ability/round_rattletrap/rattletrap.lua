...
function CheckForMaze(keys)
    ...
    if caster:GetHealth() < caster:GetMaxHealth() * 0.8 and caster:GetHealth() > caster:GetMaxHealth() * 0.5 and caster.maze_switch == 0 then
        ...
        TecMazeCreater:Init()
        local duration = 2
        local pull_end_time = GameRules:GetGameTime() + 2
        local units = FindUnitsInRadius(caster:GetTeamNumber(), Vector(0, 0, 0), nil, -1, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, FIND_ANY_ORDER, false)
